 ...
 * 原生 click；回饋高亮（.row-tap）也加在正確那列。列內按鈕（編輯/刪除/截圖，各有自己的 onclick）
 * 不接管、交給原生。只在觸控裝置啟用；桌面走原生 click。
 * ============================================================= */
(function (global) {
  'use strict';
  ...
  function clearHi(row) { if (row) { try { row.classList.remove('row-tap'); } catch (_) {} } }
  ...
  function onStart(e) {
    ...
    try {
      ...
      row.classList.add('row-tap');   // 立即回饋在正確那列
    } catch (_) { sRow = null; }
  }
  function onMove(e) {
    if (!sRow) return;
