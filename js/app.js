...
function _dispatchMenu(action) {
  ...
  else if (action === 'fuel') { closeSheet(); if (window.MaptripNearby) MaptripNearby.run('fuel'); }
  else if (action === 'parking') { closeSheet(); if (window.MaptripNearby) MaptripNearby.run('parking'); }
  else if (action === 'store') { closeSheet(); if (window.MaptripNearby) MaptripNearby.run('store'); }
  ...
  else if (action === 'flight') { closeSheet(); if (window.MaptripFlight && MaptripFlight.openLookup) MaptripFlight.openLookup(); }
...
function renderHistorySheet(keepState) {
  ...
  body.innerHTML = '<div id="trash-entry"></div>' + months.map((mk, monthIdx) => {
    ...
    const daysHtml = mDays.map(day => {
      ...
      // 換日分段（全自動，不再有手動鈕）：一日桶內若凌晨隔超過 6h → 段首前插一條分隔線提示。
      const _segs = window.MaptripDayBoundary ? MaptripDayBoundary.sessionize(trips) : [trips];
      const _segStartAfter = new Set();
      _segs.forEach((s, si) => { if (s[0] && si > 0) _segStartAfter.add(s[0]); });
