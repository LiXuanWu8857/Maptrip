...
- **GPS 飄移群清理（v246）**：都市峽谷反射／停等紅燈會產生「來回鋸齒／小圈」飄移群。
  ...
  - **停等紅燈原地飄移「圈」收斂（v276）**：`_dropSpikes` 只往前看 4 點，但紅燈久等會堆出
    「幾十個點的一小圈」（使用者實例：路口一個小方框），遠超 4 點窗、收不掉。新增 `collapseStalls`
    （geo-clean.js）：從上個保留點 a 出發，若一段路徑「在小範圍內（離 a <CAP=100m）繞了不少路
    （via>=100m）、點數>=5 後又繞回 a 附近（<35m）」＝原地飄移 → 整叢收成一點。**CAP=100m 是關鍵
    護欄**：真的繞街廓（半徑通常>100m）先超過 CAP 而不被收，只收侷限小圈。`cleanTrace` 先跑
...
- **收支報表** `js/finance.js`：月營收（排除「其他」）、淨收入＝營收−抽成/叫車−支出、
  ...
  - **v290 長條圖長按看金額**：分析頁「各時段營收／各星期營收」長條圖手機看不到金額（`title` 只在桌面 hover）。
    新增 `bindChartTips`（`render` 畫完分析頁後綁；委派在 `#finance-body` 只綁一次、跨 render 有效）：每根長條
...
- **單行道方向顯示（v1.1.355；v1.1.356 改 Google 風格箭頭）** `js/oneway.js`（MaptripOneway）：選單「➡️ 單行道方向」開關。
  **v356 依使用者提供的 Google Maps 截圖改版**：不再畫整條紫線，改成**沿每條單行道每隔約 85m 放一個灰色小箭頭 chevron**
  ...
  箭頭是**完整箭頭「↑」＝尾柄＋箭頭**（`_chevron` 回 5 點 [尾端,箭尖,左翼,箭尖回描,右翼]＝單一折線畫尾柄＋V 頭；v357 是純 chevron「‹」無尾、v358 依使用者要求補尾柄並縮小）。
  **固定公尺尺寸 `ARROW_M`＝5m（總長含尾柄，≈略小於道路寬）**：地理座標本來就隨地圖縮放，故縮小時變小（幾乎看不到）、放大時剛好貼路
  ...
  **血淚重點**：本 App 地圖朝車頭旋轉，
  ...
  純函式 `_bearing/_dest/_arrowPath/_parseWays/_overpassBody/_haversine` 供測試（node 13 項純函式＋10 項 wire：
  enable→抓取→畫線/方向反轉/縮放門檻/關閉清除，全過）。**待辦**：箭頭密度/顏色可依回饋調；未來可考慮沿長路加中段箭頭。
- **記帳者模式** `js/bookkeeper.js`：邀請碼授權；記帳者唯讀行程、可編抽成；
  ...
  **v273 每趟列排版微調**：①**車資往左靠**（`.bk-trow .f` 改 `text-align:left`，與最右欄拉開）；
  ②**最右欄改帶標籤** `_cCell`＝「抽成：X／叫車：Y」（現金抽成不適用顯示「-」、沒叫車整格「-」、非現金 0/0 也「-」）。
  `bktaxi.js` 43 項全過零 pageerror。
  **v274 每趟列表格化**：欄名（時間/付款/車資/抽成/叫車）改成**置頂 sticky 表頭** `_thead`，每一列只放數值、
...
- **底部 sheet 可拖曳展開＋總計恆置頂（v278）** `js/sheet-drag.js`（MaptripSheetDrag）：今日行程/歷史 sheet
  ...
  **v279 推廣到所有底部 sheet**：`.sheet-expanded` 由 `#trip/#history` 專屬改**通用 class**（`!important`），
  收支報表（`finance.js` open 時 `wire(finance-sheet,close)`）、批次抽成（`app.js openCommissionBatch` 時
  `wire(commission-sheet,closeCommissionBatch)`）都接上同一支拖曳邏輯；兩者 CSS 補 `transition:max-height .2s`。
  拖曳本來就即時跟手（onMove 逐幀寫 inline max-height）。測試 `sheetdrag.js` 擴到 11 項（通用 class/收支可 wire）全過。
  **v280 iOS 拖不動修復（血淚）**：使用者回報「今日/歷史往上拖沒反應」。根因＝把手/標題列**沒設
  ...
  **v285 三調（依真機回饋）**：①**閃/抖修掉**——根因＝`onStart` 對**每個**手勢（含純捲動）都關 transition/animation，
  且拖曳中 `.sheet-expanded`（直角）↔預設（圓角）切換無過渡＝圓角瞬變「閃一下」。改：`onStart` **完全不碰**
  sheet 樣式；**只在確定要動 sheet（mode 決定為 'sheet'）時**才設 `transition:'border-radius .15s ease'`（max-height
  不列＝跟手即時、border-radius 平滑不閃）；純捲動手勢從頭到尾不動 sheet。②**頂端別再偵測上滑**——已展開往上一律
  走 scroll 交給原生捲動（本來就是，配合①後不再有殘影）。③**收合門檻改「下拉低於半螢幕就關」**——`onEnd` 由固定
  `lastH<160` 改 `lastH < 0.5*innerHeight`；④**把手改回細細一條**——移除 v281 為放大點擊區加的
